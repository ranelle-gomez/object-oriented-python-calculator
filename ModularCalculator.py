...
class ModularCalculator:
    ...
    def subtract(self, numbers):
        """"Return the difference between the first inputted number and the rest of the inputted numbers.
        >>> subtract([6, 4])
        2
    ...
    def two_integer_exponentiate(self, base, exponent):
        """"Return the result of raising base to exponent, where base and exponent are integers.
        ...
        if not isinstance(base, int) or not isinstance(
                exponent, int
        ):  # imposed in case another main() not imposing integer-only.
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if choice not in "123456":
            ...
            if len(summands) < 2:
                print(
                    'Invalid number of input; takes at least two summands.\n')
                continue
            start_time = time.time()
            print(obj.add(summands))
            print('--- Calculated in %s second(s)! ---' %
                  (time.time() - start_time))
        elif choice == '2':
            args = [
                float(i) for i in input(
                    'Input the numbers you would like to subtract from the first, separated by spaces.\n'
                    'For 8 - 4 - 4: "8 4 4"\n'
            ...
            if len(multicands) < 2:
                print(
                    'Invalid number of input; takes at least two multicands.\n'
                )
                continue
            start_time = time.time()
            ...
            args = [
                int(i) for i in input(
                    'Input the dividend and divisor(s). For 6 ÷ 4 ÷ 2: "6 4 2"\n'
                ).split()
            ]
            if len(args) < 2:
                print(
                    'Invalid number of input; takes 1 dividend and 1 or more divisors.\n'
                )
                continue
            start_time = time.time()
            ...
            try:
                args = [
                    int(i) for i in input(
                        'Input the base and exponent, separated by a space. For 3^2: "3 2".\n'
                    ).split()
                ]
                base, exponent = args[0], args[1]
            except ValueError:
                print(
                    'The base and exponent must both be integers. Please try again.'
                )
                continue
            if len(args) != 2:
                print(
                    'Invalid number of input; takes only 2 arguments: 1 base and 1 exponent.\n'
                )
                continue
            start_time = time.time()
